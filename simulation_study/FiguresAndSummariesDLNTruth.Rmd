...
```{r simdat, echo=F}
simnames <- c('SimDLNTruth20251223100.Rdata', 'SimDLNTruth20251223250.Rdata', 'SimDLNTruth20251223500.Rdata', 'SimDLNTruth202512231000.Rdata')
myn <- c(100, 250, 500, 1000)
temp1 <- NULL
nmods <- 8
for(ii in 1:length(simnames)){
  load(simnames[ii])
  mymumse <- matrix(0, 100, nmods)
  mysigmamse <- matrix(0, 100, nmods)
  for(jj in 1:nmods){
    for(kk in 1:100){
      mymumse[kk,jj] <- mean((mu_estimates[,kk,jj] -mu)^2, na.rm=T)
      mysigmamse[kk, jj] <- mean((sd_estimates[,kk,jj] - sd_true)^2, na.rm=T)
    }
  }
  ...
  temp1[[ii]] <- list(dat=dat, mu_avg_interval_width=mu_avg_interval_width, mu_avg_rmse=apply(mymumse, 2, mean, na.rm=T), #(apply((mu_estimates - array(rep(mu, 1000*5), c(400, 1000, 5)))^2,3,mean, na.rm=T)),
                      ...
                      sd_coverage_rMSE=sd_coverage_rMSE, sd_marginal_coverage=apply(sd_covered, 3, mean, na.rm=T), var_true=var_true,  mu=mu, beta_true=beta_true, alpha_true=alpha_true, mu_estimates=mu_estimates, sd_obs_cov=apply(sd_covered, c(1,3), mean, na.rm=T), mu_obs_cov=apply(mu_covered, c(1,3), mean, na.rm=T))

  png(paste0("Figs/MuDLNTruthCoveragen",myn[ii],".png"), width=7.5,height=5.1,units="in", res=300)
  ...
  png(paste0("Figs/SigmaDLNTruthCoveragen",myn[ii],".png"), width=7.5,height=5.1,units="in", res=300)
